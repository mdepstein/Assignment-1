function plotting_secant_solver()
...
for i = 1:length(x0)
    ...
    % Run solver
    x_root(i) = secant_solver1(f_record,x0(i),x1(i),1000,1e-12,1e-12,100);
...
for i = 1:length(error_list)
    ...
    xlabel('e_n')
    ylabel('e_{n+1}')
    title("Secant Method Convergence Rate")
end
...
for i = 1:length(error_list)
    ...
    error_list0 = [error_list0, error(1:end-1)];
    error_list1 = [error_list1, error(2:end)];
    h(1) = loglog(error(1:end-1),error(2:end),'ro','markerfacecolor','r', 'MarkerSize', 2, 'MarkerEdgeColor', 'r');
    hold on;
    xlabel('e_n')
    ylabel('e_{n+1}')
    title("Secant Method Convergence Rate")
...
%plot filtered data
h(2) = loglog(x_regression,y_regression,'b.','linewidth',2,'Color','b', 'MarkerEdgeColor', 'b')
