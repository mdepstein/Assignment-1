%Example template for analysis function
%INPUTS:
%solver_flag: an integer from 1-4 indicating which solver to use
%
% 1->Bisection 2-> Newton 3->Secant 4->fzero
%fun: the mathematical function that we are using the
%
% solver to compute the root of
%x_guess0: the initial guess used to compute x_root
%guess_list1: a list of initial guesses for each trial
%guess_list2: a second list of initial guesses for each trial
%
% if guess_list2 is not needed, then set to zero in input
    %filter_list: a list of constants used to filter the collected data
function convergence_analysis(solver_flag, fun, x_guess0, guess_list1, guess_list2, filter_list)

%Create an instance of the input_recorder
my_recorder = input_recorder();

%Use input_recorder to generate a version of the test function
%that records the input after every iteration
%Since test_fun is defined using function keyword
f_record = my_recorder.generate_recorder_fun(fun);

% Exit parameters
dxtol = 1e-12;
ftol = 1e-12;
max_iter = 1000;
dx_max = 1e10;

%initialize lists
x_root = [];
error_list = cell(length(guess_list1),1);
flag = [];
fval = [];
[X,Y] = meshgrid(guess_list1,guess_list2);

    for i = 1:length(guess_list1)*length(guess_list2)
        if solver_flag == 1
            % If left guess is larger than right guess skip that guess
            % if guess_list1(i) <= guess_list2(i)
            %     continue
            % end

            [x_root(i), flag(i)]  = bisection_solver1(f_record, X(i), Y(i), dxtol, ftol, max_iter);

        elseif solver_flag == 2
            [x_root(i), flag(i)] = newton_solver1(f_record, guess_list1(i), max_iter, ftol, dxtol, dx_max);

        elseif solver_flag == 3
            [x_root(i), flag(i)] = secant_solver1(f_record, X(i), Y(i), max_iter, ftol, dxtol, dx_max);

        elseif solver_flag == 4
            [x_root(i), fval(i),flag(i)] = fzero(f_record, guess_list1(i));

        else
            fprintf('Enter a valid solver flag')
            return
        end
        input_list = my_recorder.get_input_list();

        % Calculate error
        error = abs(input_list-x_root(i));
        error_list{i} = error;

        % Reset recorder
        my_recorder.clear_input_list();
    end

    if solver_flag == 2
        % Predict k value (for Newton's method)
        x_r = x_root(1);
        h_step = 1e-4;
        dx = (f_record(x_r+h_step)-f_record(x_r))/(h_step);
        dx2 = (f_record(x_r+h_step)-2*f_record(x_r)-f_record(x_r-h_step))/(h_step^2);
        k_predicted = abs(0.5*dx2/dx)
    end


    %plot the inputs
    figure;
    error_list0 = [];
    error_list1 = [];

    if solver_flag == 1
        solver_title = "Bisection Method";
        x_success = X(find(flag==1));
        x_fail = X(find(flag==0));
        y_success= Y(find(flag==1));
        y_fail = Y(find(flag==0));
        hold on
        h(1) = plot(x_success,y_success, 'ko','markerfacecolor','g', 'MarkerSize', 4, 'Color','g');
        h(2) = plot(x_fail,y_fail,'ko','markerfacecolor','r', 'MarkerSize', 4, 'Color','r');
        x_root_point = mean(x_root(~isnan(x_root)));
        h(3) = plot([x_root_point x_root_point], [10 40], 'LineWidth', 3, 'Color', 'Blue');
        h(4) = plot([10 40], [x_root_point x_root_point], 'LineWidth', 3, 'Color', 'Cyan');
        h(5) = plot(x_root_point, x_root_point, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'black');
        l = legend(h,"Success", "Failure", "Root Location x", "Root Location y",  "Root Location");
        fontsize(l, 14, 'points');

        xlabel('Initial Guess Left (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        ylabel('Initial Guess Right (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        fit_line_x = 10.^[-14:.1:1.5];


    elseif solver_flag == 2
        solver_title = "Newton's Method";
        x = linspace(0,50,1000);
        y = fun(x);
        h(1) = plot(x,y,Color='k');
        x_success = guess_list1(find(flag==1));
        x_fail = guess_list1(find(flag==0));
        y_success= fun(x_success);
        y_fail = fun(x_fail);
        hold on;
        h(2) = plot(x_success,y_success, 'ko','markerfacecolor','g', 'MarkerSize', 4, 'Color','g');
        h(3) = plot(x_fail,y_fail,'ko','markerfacecolor','r', 'MarkerSize', 2, 'Color','r');
        
        x_root_point = mean(x_success(~isnan(x_success)));
        h(4) = plot([x_root_point x_root_point], [-10 10], 'LineWidth', 3, 'Color', 'Blue');
        h(5) = plot([0 50], [0 0], 'LineWidth', 3, 'Color', 'Cyan');
        h(6) = plot(x_root_point, 0, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'black');

        l = legend(h,"Function", "Success", "Fail", "Root Location x", "Root Location y",  "Root Location", "Location", "best");
        fontsize(l, 14, 'points');
        
        xlabel('Initial guess, x0 (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        ylabel('Sigmoid Functon, f(x0) (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        ylim([-4, 6])
        xlim([0 50])
        fit_line_x = 10.^[-6:.1:3];


    elseif solver_flag == 3
        solver_title = "Secant Method";
        x_success = X(find(flag==1));
        x_fail = X(find(flag==0));
        y_success= Y(find(flag==1));
        y_fail = Y(find(flag==0));
        hold on
        h(1) = plot(x_success,y_success, 'ko','markerfacecolor','g', 'MarkerSize', 4, 'Color','g');
        h(2) = plot(x_fail,y_fail,'ko','markerfacecolor','r', 'MarkerSize', 4, 'Color','r');
        x_root_point = mean(x_root(~isnan(x_root)));
        h(3) = plot([x_root_point x_root_point], [10 40], 'LineWidth', 3, 'Color', 'Blue');
        h(4) = plot([10 40], [x_root_point x_root_point], 'LineWidth', 3, 'Color', 'Cyan');
        h(5) = plot(x_root_point, x_root_point, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'black');
        l = legend(h,"Success", "Failure", "Root Location x", "Root Location y", "Root Location", "Location", "Best");
        fontsize(l, 14, 'points');

        xlabel('Initial Guess 1 (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        ylabel('Initial Guess 2 (-)', 'Interpreter', 'Latex', 'FontSize', 15)

        fit_line_x = 10.^[-8:.1:2];



    elseif solver_flag == 4
        solver_title = "FZero";
        x = linspace(0,50,1000);
        y = fun(x);
        h(1) = plot(x,y,Color='k')
        x_success = guess_list1(find(flag==1));
        x_fail = guess_list1(find(flag~=1));
        y_success= fun(x_success);
        y_fail = fun(x_fail);
        hold on
        h(2) = plot(x_success,y_success, 'ko','markerfacecolor','g', 'MarkerSize', 4, 'Color','g');
        %h(3) = plot(x_fail,y_fail,'ko','markerfacecolor','r', 'MarkerSize', 2, 'Color','r');
        xlabel('x', 'Interpreter', 'Latex', 'FontSize', 15)
        ylabel('f(x)', 'Interpreter', 'Latex', 'FontSize', 15)
        fit_line_x = 10.^[-9.5:.1:1.5];
    end
    title({solver_title} + " Initial Guess Success/Failure Plot for Sigmoid", 'Interpreter', 'Latex', 'FontSize', 20)
    figure();
    for i = 1:length(error_list)
        error = error_list{i};
        error_list0 = [error_list0, error(1:end-1)];
        error_list1 = [error_list1, error(2:end)];
        h(1) = loglog(error(1:end-1),error(2:end),'ko','markerfacecolor','r', 'MarkerSize', 4, 'Color','r');
        hold on;
        xlabel('Error at Current Iteration (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        ylabel('Error at Next Iteration (-)', 'Interpreter', 'Latex', 'FontSize', 15)
        title({solver_title} + " Convergence Rate Plot", 'Interpreter', 'Latex', 'FontSize', 20)
    end

    x_regression = [];
    y_regression = [];
    % filter_list = [1e-15, 1e-2, 1e-14, 1e-2, 2];
    index_list = 1:length(error_list0);
    %iterate through the collected data
    for n=1:length(index_list)
        %if the error is not too big or too small
        %and it was enough iterations into the trial...
        if error_list0(n)>filter_list(1) && error_list0(n)<filter_list(2) && ...
                error_list1(n)>filter_list(3) && error_list1(n)<filter_list(4) && ...
                index_list(n)>filter_list(5)
            %then add it to the set of points for regression
            x_regression(end+1) = error_list0(n);
            y_regression(end+1) = error_list1(n);
        end
    end

    h(2) = loglog(x_regression,y_regression,'ko','markerfacecolor','b', 'MarkerSize', 4, 'Color','b')

    %x_regression -> e_n
    %y_regression -> e_{n+1}
    [p,k] = generate_error_fit(x_regression,y_regression)

    %example for how to plot fit line
    %generate x data on a logarithmic range
    %compute the corresponding y values
    fit_line_y = k*fit_line_x.^p;
    %plot on a loglog plot.
    h(3) = loglog(fit_line_x,fit_line_y,'k-','linewidth', 2,'Color','black');
    set(gca, 'XMinorTick', 'off');
    set(gca, 'YMinorTick', 'off');
    
    l = legend(h,"Raw Data", "Filtered Data", "Fit Line");
    fontsize(l, 14, 'points');

    set(l,'location','northwest');
    fontsize(l, 14, "points"); % Sets the legend font size to 14 points
end

    
    %example for how to compute the fit line
    %data points to be used in the regression
    %x_regression -> e_n
    %y_regression -> e_{n+1}
    %p and k are the output coefficients
function [p,k] = generate_error_fit(x_regression,y_regression)
    %generate Y, X1, and X2
    %note that I use the transpose operator (')
    %to convert the result from a row vector to a column
    %If you are copy-pasting, the ' character may not work correctly
    Y = log(y_regression)';
    X1 = log(x_regression)';
    X2 = ones(length(X1),1);
    %run the regression
    coeff_vec = regress(Y,[X1,X2]);
    %pull out the coefficients from the fit
    p = coeff_vec(1)
    k = exp(coeff_vec(2))
end

