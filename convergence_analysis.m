...
[X,Y] = meshgrid(guess_list1,guess_list2);
    ...
    if solver_flag == 1
        ...
        xline(x_root, 'LineWidth', 3, 'Color', 'Blue');
        yline(x_root, 'LineWidth', 3, 'Color', 'Blue');
        x_root_point = mean(x_root(~isnan(x_root)));
        h(3) = plot(x_root_point, x_root_point, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'black');

        l = legend(h,"Success", "Failure", "Root Location");
    ...
    elseif solver_flag == 2
        ...
        y_fail = fun(x_fail);
        hold on        h(2) = plot(x_success,y_success, 'ko','markerfacecolor','g', 'MarkerSize', 4, 'Color','g');
        
        % h(3) = plot(x_fail,y_fail,'ko','markerfacecolor','r', 'MarkerSize', 2, 'Color','r');
        l = legend(h,"Function", "Success", "Failure", 'Root Location');
